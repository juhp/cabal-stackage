...
generateProjectFile :: Bool -> FilePath -> [String] -> IO FilePath
generateProjectFile debug configPath userConstraints = do
  ...
  if null userConstraints
    then do
      ...
      when debug $ warning $ "Project file: " ++ stackageProjectFile
    else do
      ...
      when debug $ warning $ "Config file: " ++ stackageConfigFile
                             ++ " (" ++ show (length filtered) ++ " constraints"
                             ++ ", " ++ show (length userConstraints) ++ " overridden)"
      writeFile stackageProjectFile $ unlines $
        preamble
