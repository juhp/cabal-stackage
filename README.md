...
`cabal-stackage.project` is written to your project root on every run and
...
When package constraints are active, a `cabal-stackage.config` file is also
written to the project root.  It contains the filtered Stackage constraints
(with the overridden packages removed) and is imported by
`cabal-stackage.project`, which then adds the user override constraints as a
separate stanza:

```
-- cabal-stackage.config
-- Generated by cabal-stackage, do not edit
constraints: abstract-deque ==0.3,
             ...

-- cabal-stackage.project
-- Generated by cabal-stackage, do not edit
packages: .
import: cabal-stackage.config
constraints: aeson ==2.1.0.0
```
